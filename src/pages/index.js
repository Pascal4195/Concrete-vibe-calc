...
function encodeConvertToAssets(decimals) {
  // Build 10^decimals in hex without BigInt — safe for all browsers
  // We compute it as a decimal string first then convert
  var val = '1';
  for (var i = 0; i < decimals; i++) val += '0'; // e.g. decimals=6 -> "1000000"
  // Convert decimal string to hex manually to avoid BigInt
  var num = val;
  var hex = '';
  // Use the browser's Number for values up to 18 decimals (safe up to 10^18)
  // WBTC=8, USDT=6 are well within Number range; WETH/frxUSD=18 is exactly safe
  var n = parseFloat(num);
  hex = Math.floor(n).toString(16);
  return SEL.convertToAssets + hex.padStart(64, '0');
}

function decodeUint256(hex) {
  if (!hex || hex === '0x') return 0;
  // Parse hex string as a float — sufficient precision for price ratio comparison
  // We only use this for APY ratio math, not token accounting, so float is fine
...
async function fetchSingleVault(config) {
  ...
  var tvlRaw = totalAssets / Math.pow(10, assetDecimals);

  return Object.assign({}, config, {
    totalAssets:   totalAssets,
    totalSupply:   totalSupply,
    pricePerShare: priceNow / Math.pow(10, assetDecimals),
    tvlRaw:        tvlRaw,
...
function LiveBadge({ loading, lastFetched, onRefresh }) {
  // Tick every second so timeSince() re-evaluates and the counter moves
...
function VaultCard({ vault, selected, onClick }) {
  ...
  // Purple accent for institutional vaults
  if (institutional) borderColor = '#A855F7';
    ...
    if (institutional) {
      return (
        <div className="text-right">
          <div
            className="font-mono text-[11px] font-black px-2 py-0.5 border tracking-widest"
            style={{
              color: '#A855F7',
    ...
    >
      ...
      {institutional && (
        <div
          ...
          style={{ borderColor: '#A855F7', background: 'rgba(168,85,247,0.06)', color: '#A855F7', opacity: 0.8 }}
        >
      ...
      <div className="flex flex-wrap gap-3 text-xs font-mono opacity-40 text-[#00FF41]">
        ...
        {institutional && (
          <span style={{ color: '#A855F7' }}>TVL: $400M+</span>
        )}
        {pending && (
...
export default function Home() {
    ...
    return (
      <div style={{
        ...
        <style>{`
          @keyframes heartbeat {
            0%, 100% { opacity: 0.2; }
            50% { opacity: 1; text-shadow: 0 0 12px #00FF41; }
          }
        `}</style>
      </div>
    );
  ...
  // Tick every second so the REFRESHED counter in the stat bar updates live
  var nowTickState = useState(Date.now());
  var setNowTick   = nowTickState[1];
  useEffect(function() {
    var t = setInterval(function() { setNowTick(Date.now()); }, 1000);
    return function() { clearInterval(t); };
  }, []);
